...
Currently we can recommend the formally verified [SafeMultisig](../../multisignature-wallet.md) contract for use in deposit accounts. It is well tested and secure, supports multiple custodians, and can be set up to require several independent signatures for any transfers. However it has certain limitations, that may prove problematic for exchanges: it is not possible to send tokens transfers in batches to multiple addresses.
...
The recommended [SafeMultisig](../../multisignature-wallet.md) contract is used.

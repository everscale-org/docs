...
`inbound_external_message_fee` — is deducted, if an [inbound external message](fee-calculation.md#inbound-external-messages) is imported in the transaction.

`storage_fees` — [storage costs](fee-calculation.md#storage-fees) since the moment of the last transaction.

`gas_fees` — include all gas fees associated with the transaction. You can find more info in the [Gas calculation basics](managing-gas.md#gas-calculation-basics) section.

`total_action_fees` — fees for performing _send message_ [actions](fee-calculation.md#action-fees).

`outbound_internal_messages_fee` — is calculated as a sum of fees for all [outbound internal messages](fee-calculation.md#outbound-messages) generated by the transaction.
...
Whenever an inbound external message needs to be imported for transaction execution, the for this action fee is calculated according to the [standard forwarding fee formula](fee-calculation.md#message-fees), and paid to the current validators.
...
`trans.gas_fees` include all gas fees associated with the transaction. You can find more information in the [Gas calculation basics](managing-gas.md#gas-calculation-basics) section.

...
* `Storage` - contract that store token byte content. Storage is independent. Storage don't store `NFT` address because `NFT` contract address can be changed by burning and redeployment from another collection

## Collection
Every [TIP-4.1](1.md) `Collection` contract must implement `TIP4x4Collection`
```solidity
pragma ton-solidity >= 0.58.0;

interface TIP4x4Collection {
...
### TIP4x4Collection.resolveStorage()
```solidity
...
## NFT
Every [TIP-4.1](1.md) `NFT` contract must implement `TIP4x4NFT`
```solidity
pragma ton-solidity >= 0.58.0;
...
### TIP4x4NFT.onStorageFillComplete()
```solidity
function onStorageFillComplete(address gasReceiver) external;
```
* `gasReceiver` (`address`) - address of contract that receive all remaining contract balance then last chunk filled
Calling the `Storage.fill()` on storage contract that fills the last chunk should call `TIP4x4NFT.onStorageFillComplete()` on token contract

### TIP4x4NFT.getStorage()
```solidity
function getStorage() external view responsible returns (address addr);
```
* `addr` (`address`) - storage contract address
...
## Storage
Every `Storage` contract must implement `TIP4x4Storage`
```solidity
pragma ton-solidity >= 0.58.0;

interface TIP4x4Storage {
...
* `bytes` (`chunk`) - data. Max size of data limited by external message payload size. Maximum size external message payload size is `16KB` at 2022-03-18
* `gasReceiver` (`address`) - address of contract that receive all remaining contract balance then last chunk filled

### TIP4x4Storage.getInfo()
```solidity
function getInfo() external view returns (
    address nft,
    address collection,
...
* `chunks` (`mapping(uint8 => bytes)`) - byte content. Maximum content size is `256 chunks * chunk size`. Max size of data limited by external message payload size. Maximum size external message payload size is `16KB` at 2022-03-18 Maximum content size is `256 * 16KB = 4MB` at 2022-03-18

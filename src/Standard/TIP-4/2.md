---
title: 2. Liquid Token Base
sidebar_position: 2
---

# Non-Fungible Token - Liquid Token Base (TIP-4.2)
...
`LiquidTokenBase` - Contract that represents a single non-fungible `Token`. Following distributed programming paradigm, one `Token` is one contract. `Token` stores general information and metadata in JSON format. Metadata is always stored on-chain.

### LiquidCollectionBase

`LiquidCollectionBase` - Contract that represents `Collection` that can mint `Tokens`. 

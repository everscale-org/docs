...
Every TIP4.1 compliant collection contract must implement the `ITIP4_1Collection` interface and [TIP-6.1](./../TIP-6/1.md) interfaces
...
Owner and manager are blockchain addresses that own and control the NFT. Owner address and manager address can be same or different.

Every TIP4.1 compliant NFT contract must implement the `ITIP4_1NFT` interface and [TIP-6.1](./../TIP-6/1.md) interfaces
...
interface ITIP4_1NFT {
    ...
    /// It invoked from manager address only
    /// @dev Change manager address to newOwner address too
    /// @dev Emit OwnerChanged and ManagerChanged events
...
* `owner` (`address`) - The initial\last owner of the NFT
* `manager` (`address`) - The initial\last manager of the NFT
* `collection` (`address`) - The initial address who initiate NFT deploy

This events emits when NFTs are created, burned or moved to new onwer\manager.
...
See the [Events](#Events) documentation for your responsibilities when creating NFT.

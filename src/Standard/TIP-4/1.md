...
NFTs is a smart contracts deployed from `collection` smart contract use nftCode and id. 
...
The contract represent information about current NFT and control logic. Each NFT contains two roles
- owner logic.
- manager logic.

Owner address it's address that own this NFT. Owner can see NFT in wallets, marketplaces, apps. Owner can use the NFT for authorize, for proof in games etc.

Manager address it's address that control this NFT. Manager can burn NFT, can change owner or change manager (transfer manager role).

Owner address and manager address can be same usally. When NFT put up to sell it's means that manager address setted to smartcontract address with sell logic. It's same for farming logic or custom logic for use NFT.

Good practices is set manager address to new owner address when owner changed.
...
Change NFT owner. You must emit `OwnerChanged` events when NFT owner changed. The NFT send callbacks if `callbacks` not empty.

### ITIP41NFT.changeManager()
```solidity
function changeManager(address newManager, address sendGasTo, mapping(address => CallbackParams)  callbacks) external;
```
...
Change NFT manager. You must emit `ManagerChanged` events when NFT owner changed. The NFT send callbacks if `callbacks` not empty.
...
This events emits when NFTs are created, burned or moved to new owner\manager.

### Mint NFT

A functions and constructor signature is not included in the specification. 

The NFT must deployed from collection smart coontract.
...
The NFT must send internal message to collection contract before NFT burned. 
...
<img src="images/collection.svg" width="100%">

### Minting
<img src="images/mint.svg" width="100%">

### Burning
<img src="images/burn.svg" width="100%">

### Change owner
<img src="images/changeOwner.svg" width="100%">

### Change manager
<img src="images/changeManager.svg" width="100%">

### Example how to use NFT. Put on sell using `changeManager()`
<img src="images/sell.svg" width="100%">

### Example how to use NFT. Buy using `changeOwner()`
<img src="images/buy.svg" width="100%">

### Example how to use NFT. Put on sell by [TIP-3.1](../TIP-3/1.md) tokens
<img src="images/tip3sell.svg" width="100%">

### Example how to use NFT. Buy for [TIP-3.1](../TIP-3/1.md) tokens
<img src="images/tip3buy.svg" width="100%">

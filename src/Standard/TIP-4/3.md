...
This make blockchain application less dependent on different off-chain parsers and indexers

On-chain Indexes is universal solution to easy and fast searching any data in blockchain.
This document show standard for basic query.
Any developer can get idea of this solution and realize own on-chain index
...
## Collection
* [TIP-4.1](1.md) `Collection` contract must implement the `TIP4x3Collection` interface and [TIP-6.1](./../TIP-6/1.md) interfaces
...
## NFT
* [TIP-4.1](1.md) `Collection` contract must implement the `TIP4x3NFT` interface and [TIP-6.1](./../TIP-6/1.md) interfaces
...
#### TIP4x3NFT.indexCodeHash()
```solidity
function indexCodeHash() external view responsible returns (uint256 hash);
```
* `hash` (`uint256`) - basis index contract code hash

#### TIP4x3NFT.resolveIndex()
```solidity
function resolveIndex(address collection, address owner) external view responsible returns (address index);
```
* `collection` (`address`) - collection token contract address

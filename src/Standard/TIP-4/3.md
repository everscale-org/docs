...
To deploy an index, you must add **salt** to the code with the `stamp` (`string`).
`stamp` is required to get a **unique code hash**.
The **unique code hash** is required to avoid collisions with other index contracts that are used to indexing other types of contracts
